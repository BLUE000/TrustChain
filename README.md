...
TrustChainを導入したアプリで、機能追加や修正などのアップデート作業を行う際のポイントです。

*   **ブランチを分けて開発するとき**
    新機能などのために `git checkout -b` で別ブランチを作って開発する場合でも、**「コミットした変更をリモート（GitHub）に `push` してからビルドする」** だけで、改ざん検知（ウォーターマーク）を自動で回避できます。
*   **手元でコードを書き換え中のとき**
    未コミットの変更がある状態でビルドすると、一時的に改ざん判定（ウォーターマーク表示ルート）になります。ウォーターマークなしで動作確認したい場合は、一度変更をコミットして `push` してからビルドを行ってください。
