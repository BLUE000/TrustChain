...
### ST_TC_SYS_006: バイナリ透かしの抽出および復号テスト (BinMarkManager)
* **テスト環境前提**:
  * 公式リリース用にビルドされ、透かしが挿入されたバイナリ（EXE）が存在する。
  * `BinMarkManager` ツール（GUI版）がテスト環境に用意されており、開発者用の復号鍵（Obfuscation Key）を所持している。
* **試験手順**:
  1. `BinMarkManager` のGUIツールを起動する。
  2. 対象のビルド済みバイナリをドラッグ＆ドロップで読み込ませ、正しい復号鍵を入力して「Extract & Decrypt Watermark」を実行する。
* **期待される結果**:
  1. ツールによってバイナリ末尾からテキストタグで囲まれた暗号化データと平文コピーライトが正常に抽出されること。
  2. TransCipher 3.0.0 の復号プロセス（TLV解析含む）が成功し、オリジナルのルートコミットハッシュと `BLUE002` の署名が平文として画面に表示されること。
